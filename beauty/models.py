...
class BeautyPostQuerySet(models.QuerySet):
    def published(self):
        return self.filter(published=True)


class BeautyPostManager(models.Manager):
    """
    Manage for the Comment section
    """
    def get_beautypost_qs(self):
        return BeautyPostQuerySet(self.model, using=self._db)

    def published(self):
        return self.get_beautypost_qs().published()
    ...
    def filter_by_instance(self, instance):
        ...
        obj_id = instance.id
        qs = super(BeautyPostManager, self).filter(content_type=content_type, object_id=obj_id)
        return qs
...
def get_upload_path(instance, filename):
    ...
    return os.path.join("uploads/beauty", no_space_title, filename)
...
def get_image_upload_path(instance, filename):
    ...
    :return:
    """
    no_space_title = str(instance.beautypost.title.replace(' ', ''))
...
class BeautyPost(models.Model):
    ...
    def get_absolute_url(self):
        return reverse("beautypost_detail", kwargs={"pk": self.pk})
...
def create_beauty_slug(instance, new_slug=None):
    """
    ...
    if new_slug is not None:
        slug = new_slug
    qs = Beauty.objects.filter(slug=slug).order_by("-id")
    exists = qs.exists()
    if exists:
        new_slug = "%s-%s" %(slug, qs.first().id)
        return create_beauty_slug(instance, new_slug=new_slug)
    return slug
