...
class TravelPostQuerySet(models.QuerySet):
    def published(self):
        return self.filter(published=True)


class TravelPostManager(models.Manager):
    """
    Manage for the Comment section
    """
    def get_travelpost_qs(self):
        return TravelPostQuerySet(self.model, using=self._db)

    def published(self):
        return self.get_travelpost_qs().published()
    ...
    def filter_by_instance(self, instance):
        ...
        obj_id = instance.id
        qs = super(TravelPostManager, self).filter(content_type=content_type, object_id=obj_id)
        return qs
...
class TravelPost(models.Model):
    ...
    def get_absolute_url(self):
        return reverse("travelpost_detail", kwargs={"pk": self.pk})
...
class Image(models.Model):
    ...
    travelpost = models.ForeignKey(TravelPost, null=True, blank=True, on_delete=models.CASCADE)
...
def create_travel_slug(instance, new_slug=None):
    """
    ...
    if new_slug is not None:
        slug = new_slug
    qs = TravelPost.objects.filter(slug=slug).order_by("-id")
    exists = qs.exists()
    if exists:
        new_slug = "%s-%s" %(slug, qs.first().id)
        return create_travel_slug(instance, new_slug=new_slug)
    return slug
...
pre_save.connect(pre_save_post_receiver, sender=TravelPost)
