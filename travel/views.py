...
def travelpost_list(request):
    ...
    travel_posts = TravelPost.objects.published()
    paginator = Paginator(travel_posts, 6)  # Show 3 contacts per page

    page_number = request.GET.get('page')

    if page_number is not None:
        pagnated_travel_posts = paginator.page(page_number)
    else:
...
def travelpost_detail(request, pk):
    ...
    instance = get_object_or_404(TravelPost, pk=pk)
